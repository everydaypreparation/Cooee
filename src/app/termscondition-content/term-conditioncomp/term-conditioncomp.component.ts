...
@Component({
  selector: 'app-term-conditioncomp',
  templateUrl: './term-conditioncomp.component.html',
  styleUrls: ['./term-conditioncomp.component.scss']
})
export class TermConditioncompComponent implements OnInit {
  ...
  addFile() {
    if (this.fileuploadform.value.file == null) {
      ...
      this.userService.addFileTermConditionUpload(this.fileuploadform.value).subscribe(
        res => {
          if (res.status == 200) {
